...
public class RiskModelSampleRiskDistribution {
    ...
    public JSONArray getRiskHistogramValuesJSON(String fileName){
        ...
        JSONObject atRisks = new JSONObject();
        for (int i = 0; i < this.recordsAtRisk.length;i++){
            atRisks.put("bin" + i ,this.recordsAtRisk[i]);
        }

        JSONObject cumRiskObject = new JSONObject();
        cumRiskObject.put("CumulativeRisk", cumulativeRisks);

        JSONObject atRiskObject = new JSONObject();
        atRiskObject.put("AtRisk", atRisks);

        JSONArray jsonObjects = new JSONArray();
        jsonObjects.add(cumRiskObject);
        jsonObjects.add(atRiskObject);

        return jsonObjects;
    }

    public void printJSONObjects(JSONArray jsonObjects, String fileName){
